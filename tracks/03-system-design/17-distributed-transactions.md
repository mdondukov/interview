...
- **Follow-up:** "А как вы гарантируете целостность данных?" → Saga, Event Sourcing
...
- **Follow-up:** "Почему не использовать 2PC везде?" → Масштабируемость
...
- **Follow-up:** "Что лучше: Orchestration или Choreography?" → Зависит от сложности
...
- **Follow-up:** "Как вы выбирали бы между ними?" → Начните с Orchestration, переходите на Choreography по мере роста
...
- **Follow-up:** "Что если компенсация тоже упадёт?" → Нужен retry или manual intervention
...
- **Follow-up:** "Как очищать дедупликационный набор?" → TTL, например 24 часа
...
- **Follow-up:** "Что делать, если компенсация тоже упала?" → Manual intervention
...
- **Follow-up:** "Что если процессор упал после обработки, но до подтверждения?" → Нужен status check
...
- **Follow-up:** "Что если два пользователя одновременно забронировали последний номер?" → Нужна оптимистичная блокировка или версионирование
...
- **Follow-up:** "Почему Workflow должен быть детерминирован?" → Потому что Temporal воспроизводит его из истории
