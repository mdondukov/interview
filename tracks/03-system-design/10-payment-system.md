...
- Follow-up: «Как обработать просроченный idempotency key?» → Установить TTL на запись в БД (7 дней для соответствия PCI), старые ключи не обрабатываются повторно.
- Follow-up: «Что если Redis недоступен?» → Fallback на БД только (медленнее, но надёжно).
...
- Follow-up: «Как обнаружить discrepancy между реестром и платежами?» → ежедневная сверка (reconciliation).
- Follow-up: «Как обрабатывать несколько валют?» → отдельные счета и реестры для каждой валюты.
...
- Follow-up: «Как обработать, если Stripe подтвердил возврат, но наша БД упала?» → Webhook от Stripe придёт позже, будет идемпотентным.
- Follow-up: «Почему частичные возвраты требуют особой логики?» → Нужно отслеживать сумму, которую ещё можно вернуть.
...
- Follow-up: «Как обработать unmatched external платёж?» → Это может быть мошеничество или ошибка в коде; требует ручного расследования.
- Follow-up: «Зачем проверять статус, если платёж был успешен локально?» → Провайдер может отозвать платёж (chargeback, fraud); статус может измениться.
...
- Follow-up: «Как обрабатывать сохранённые карты клиента (для подписки)?» → Stripe сохраняет токен, ты запрашиваешь по токену, не по номеру.
- Follow-up: «Что делать при penetration testing?» → Разрешить безопасный тестинг, исправить найденные уязвимости.
...
- Follow-up: «Как тестировать failover без падения реального провайдера?» → Mock провайдер, inject ошибки.
...
- Follow-up: «Как тестировать retry без reальных ошибок?» → Inject ошибки в тестах, используй mocking.
...
- Follow-up: «Как обработать webhook если URL недоступен?» → Retry с exponential backoff, потом DLQ.
...
- Follow-up: «Как уменьшить chargebacks?» → Fraud detection, customer verification, clear refund policy.
...
- Follow-up: «Как тестировать webhook'и локально?» → Stripe CLI для forwarding webhooks в localhost.
